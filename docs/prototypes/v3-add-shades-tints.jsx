...
function ColorHarmonies({ hex }) {
  ...
  const splitComp1Hsl = { ...hsl, h: (hsl.h + 150) % 360 };
  const splitComp1Rgb = hslToRgb(splitComp1Hsl.h, splitComp1Hsl.s, splitComp1Hsl.l);
  const splitComp1 = rgbToHex(splitComp1Rgb.r, splitComp1Rgb.g, splitComp1Rgb.b);
  
  const splitComp2Hsl = { ...hsl, h: (hsl.h + 210) % 360 };
  const splitComp2Rgb = hslToRgb(splitComp2Hsl.h, splitComp2Hsl.s, splitComp2Hsl.l);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="mb-6">
        <h3 className="font-semibold text-gray-700 mb-3">Split Complementary (Softer contrast)</h3>
        <div className="grid grid-cols-3 gap-4">
          <ColorSwatch hexColor={hex} label="Your Color" />
          <ColorSwatch hexColor={splitComp1} label="Split Comp +150°" description="Balanced contrast" />
          <ColorSwatch hexColor={splitComp2} label="Split Comp +210°" description="Balanced contrast" />
        </div>
      </div>
      ...
      <div className="mt-4 p-4 bg-blue-50 rounded-lg">
        <p className="text-sm text-gray-700">
          <strong>Tip:</strong> Complementary colors create bold contrast. Split complementary offers contrast with more harmony. Analogous colors create smooth, cohesive palettes. Triadic colors offer vibrant variety while maintaining balance.
        </p>
      </div>
    </div>
  );
...
// Tint and Shade Generator Component
function TintShadeGenerator({ hex }) {
  const hexToRgb = (h) => {
    const r = parseInt(h.slice(0, 2), 16);
    const g = parseInt(h.slice(2, 4), 16);
    const b = parseInt(h.slice(4, 6), 16);
  ...
  const generateTintsAndShades = () => {
    const variants = [];
    
    // Very light tints
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r + (255 - rgb.r) * 0.95),
        Math.round(rgb.g + (255 - rgb.g) * 0.95),
        Math.round(rgb.b + (255 - rgb.b) * 0.95)
      ),
      label: '95% Lighter',
    ...
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r + (255 - rgb.r) * 0.85),
        Math.round(rgb.g + (255 - rgb.g) * 0.85),
        Math.round(rgb.b + (255 - rgb.b) * 0.85)
      ),
      label: '85% Lighter',
      description: 'Very light tint'
    });
    
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r + (255 - rgb.r) * 0.7),
        Math.round(rgb.g + (255 - rgb.g) * 0.7),
        Math.round(rgb.b + (255 - rgb.b) * 0.7)
      ),
      label: '70% Lighter',
    ...
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r + (255 - rgb.r) * 0.5),
        Math.round(rgb.g + (255 - rgb.g) * 0.5),
        Math.round(rgb.b + (255 - rgb.b) * 0.5)
      ),
      label: '50% Lighter',
      description: 'Medium tint'
    });
    
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r + (255 - rgb.r) * 0.3),
        Math.round(rgb.g + (255 - rgb.g) * 0.3),
        Math.round(rgb.b + (255 - rgb.b) * 0.3)
      ),
      label: '30% Lighter',
    ...
    variants.push({
      ...
      label: '30% Darker',
      description: 'Subtle shade'
    });
    
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r * 0.5),
        Math.round(rgb.g * 0.5),
        Math.round(rgb.b * 0.5)
      ),
      label: '50% Darker',
    ...
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r * 0.3),
        Math.round(rgb.g * 0.3),
        Math.round(rgb.b * 0.3)
      ),
      label: '70% Darker',
    ...
    variants.push({
      hex: rgbToHex(
        Math.round(rgb.r * 0.15),
        Math.round(rgb.g * 0.15),
        Math.round(rgb.b * 0.15)
      ),
      label: '85% Darker',
      description: 'Very dark shade'
    });
    
    return variants;
  };
  
  const variants = generateTintsAndShades();
  
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-2">Tints & Shades Scale</h2>
      <p className="text-sm text-gray-600 mb-4">Pure color mixing (not opacity) - perfect for creating a consistent website color palette</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {variants.map((variant, index) => (
          <div key={index} className="text-center">
            ...
            />
            <div className="font-mono text-xs font-semibold text-gray-700">#{variant.hex}</div>
            <div className="text-xs text-gray-500 mt-1">{variant.label}</div>
      ...
      <div className="mt-4 p-4 bg-green-50 rounded-lg">
        <p className="text-sm text-gray-700">
          <strong>For websites:</strong> Use these for backgrounds, borders, hover states, and text variations. Tints (lighter) work for backgrounds and subtle elements. Shades (darker) work for text, borders, and emphasis.
        </p>
      </div>
    </div>
  );
